...
**Responsibility**: DelegatingHandler that automatically adds correlation-id to HTTP request headers and propagates W3C trace context. It can create OpenTelemetry child Activities (spans) for distributed tracing depending on framework and configuration.
...
3. **Output Format**: Depends on your logger configuration. For Serilog JSON output, use a JSON formatter/sink (for example, `Traceability.Logging.JsonFormatter`). `WithTraceabilityJson()` configures enrichers and is designed to work well with JSON output, but does not force a JSON sink by itself.

**JSON Output (recommended)**:

Traceability provides helpers for structured logging, but the output format depends on your sink/formatter/provider.

When using Serilog, `Traceability.Logging.JsonFormatter` can be used to emit JSON logs. The default JSON format typically includes:
