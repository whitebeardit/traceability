...
Traceability provides enrichers and helpers to keep logs consistent, but **the output format depends on your logging setup**.

- For **Serilog JSON output**, configure a JSON formatter/sink (for example, `Traceability.Logging.JsonFormatter`).
- For **Microsoft.Extensions.Logging**, the console formatting depends on the configured provider and options.
